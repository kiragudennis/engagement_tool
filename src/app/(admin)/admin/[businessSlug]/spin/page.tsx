// src/app/(admin)/admin/[businessSlug]/spin/page.tsx
...
export default function SpinningWheelAdmin() {
  ...
  const [queueGameId, setQueueGameId] = useState<string | null>(null);
  ...
  const loadQueue = async (gameId: string) => {
    setQueueLoading(true);
    try {
      const { data, error } = await supabase.rpc("get_spin_queue", {
        p_game_id: gameId,
      });
      if (error) throw error;
  ...
  const handleCallNext = async () => {
    if (!queueGameId) return;
    setCallingNext(true);
    try {
      const { data, error } = await supabase.rpc("call_next_spin", {
        p_game_id: queueGameId,
      });
      if (error) throw error;
      if (!data.success) {
        toast.error(data.error || "No one in queue");
        return;
      }
  ...
  const handleSkipCurrent = async () => {
    if (!queueGameId) return;
    try {
      const { data, error } = await supabase.rpc("skip_current_spinner", {
        p_game_id: queueGameId,
      });
      if (error) throw error;
      if (!data.success) {
        toast.error(data.error || "No current spinner");
        return;
      }
  ...
  const toggleQueue = async (gameId: string, currentEnabled: boolean) => {
    ...
    if (error) {
      ...
      toast.success(`Queue ${!currentEnabled ? "enabled" : "disabled"}`);
      await fetchGames();
    }
  };
